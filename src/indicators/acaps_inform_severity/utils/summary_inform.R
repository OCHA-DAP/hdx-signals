...
summary <- function(df_alerts, df_wrangled, df_raw) {
  ...
  df_risk <- df_risk |> dplyr$group_by(iso3, crisis_id, date) |>
  ...
  df_info <- df_info |> dplyr$full_join(df_risk, by=c("iso3", "crisis_id", "date")) |> dplyr$select(c('iso3', 'date', "text", "text2"))
