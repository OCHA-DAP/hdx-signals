...
food_insecurity_ts <- function(df_wrangled, df_raw, title, date) {
  # Add error handling and return NULL if there's an issue
  tryCatch(
    {
    ...
    error = function(e) {
      # If there's an error in plot generation, return NULL
      warning(paste("Error generating plot for", unique(df_wrangled$iso3), ":", e$message))
      return(NULL)
    }
  )
}
